...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Length of the audio(sr) seconds based on model's sr.",
    )
    parser.add_argument(
        "-d",
        "--device",
...
def main(opts=None):
    ...
    try:
        ...
        fatal(error.args[0])
    args.audiolength = args.audiolength * model.samplerate
    ...
    for batch, means, stds, tracks in tqdm(dataloader):
        ...
        # def save_sources(k, sources):
            ...
            if args.stem is None:
                ...
                stem.parent.mkdir(parents=True, exist_ok=True)
                if args.sample_rate is not None and model.samplerate != args.sample_rate:
                    other_stem = librosa.resample(
                        other_stem.detach().cpu().numpy(),
                        orig_sr=model.samplerate,
                        target_sr=args.sample_rate,
        ...
        # mp_list = [
        #     mp.Process(
        #         target=save_sources, args=(process_index, b_sources[process_index].detach().cpu())
        #     )
        #     for process_index in range(min(8, len(b_sources)))
        # ]
        ...
        #     pool.starmap(save_sources, enumerate(list(b_sources)))
